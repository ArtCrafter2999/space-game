...
func _unhandled_input(event) -> void: 
	...
	event is InputEventMouseMotion:
		...
		_y_rotation = -motionEvent.relative.x * 0.01 * sensativity
		head_rotation = -motionEvent.relative.y * 0.01 * sensativity
...
func _process(delta: float) -> void:
	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		if(head_rotation):
			if(gravity_limit):
				global_rotate(transform.basis.x, head_rotation);
			else:
				neck.rotate_x(head_rotation)
...
func _physics_process(delta):
	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		# TODO: Треба пофіксити поворот, щоб він був в process
		if(_y_rotation):
			global_rotate(transform.basis.y, _y_rotation);
			_y_rotation=0;
